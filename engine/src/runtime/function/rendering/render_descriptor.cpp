...
auto RenderDescriptorSetLayout::Builder::AddBinding(
        uint32_t binding_index,
        VkDescriptorType descriptor_type,
        VkShaderStageFlags stage_flags,
        uint32_t count) -> RenderDescriptorSetLayout::Builder & {
...
auto RenderDescriptorSetLayout::Builder::Build() const -> std::unique_ptr<RenderDescriptorSetLayout> {
    return std::make_unique<RenderDescriptorSetLayout>(m_render_device, bindings);
}

// *************** Descriptor Set Layout *********************

RenderDescriptorSetLayout::RenderDescriptorSetLayout(
        std::shared_ptr<RenderDevice> render_device, const std::unordered_map<uint32_t, VkDescriptorSetLayoutBinding> &bindings)
...
auto RenderDescriptorPool::Builder::AddPoolSize(
        VkDescriptorType descriptor_type, uint32_t count) -> RenderDescriptorPool::Builder & {
    m_pool_sizes.push_back({descriptor_type, count});
    m_current_sets_count += count;
    return *this;
}

auto RenderDescriptorPool::Builder::SetPoolFlags(
        VkDescriptorPoolCreateFlags flags) -> RenderDescriptorPool::Builder & {
    m_pool_flags = flags;
    return *this;
}
...
auto RenderDescriptorPool::Builder::Build() const -> std::unique_ptr<RenderDescriptorPool> {
    SATURN_ASSERT(m_current_sets_count > 0, "Current descriptorsets count is zero!")
    return std::make_unique<RenderDescriptorPool>(m_render_device, m_current_sets_count, m_pool_flags, m_pool_sizes);
}
...
auto RenderDescriptorPool::AllocateDescriptor(VkDescriptorSetLayout descriptor_set_layout, VkDescriptorSet &descriptor_set) const -> bool {
...
void RenderDescriptorPool::FreeDescriptors(std::vector<VkDescriptorSet> &descriptor_sets) const {
...
RenderDescriptorWriter::RenderDescriptorWriter(std::shared_ptr<RenderDescriptorSetLayout> set_layout, std::shared_ptr<RenderDescriptorPool> pool)
    : m_set_layout{std::move(set_layout)}, m_pool{std::move(pool)}, m_bindings(set_layout->GetBindings()) {}

auto RenderDescriptorWriter::WriteBuffer(
        uint32_t binding, VkDescriptorBufferInfo *buffer_info) -> RenderDescriptorWriter & {
...
auto RenderDescriptorWriter::WriteImage(
        uint32_t binding, VkDescriptorImageInfo *image_info) -> RenderDescriptorWriter & {
    SATURN_ASSERT(m_bindings.count(binding) == 1, "Layout does not contain specified binding");
...
auto RenderDescriptorWriter::Build(VkDescriptorSet &set) -> bool {
...
void RenderDescriptorWriter::Overwrite(VkDescriptorSet &set) {
    ...
    vkUpdateDescriptorSets(m_pool->m_render_device->GetVkDevice(), m_writes.size(), m_writes.data(), 0, nullptr);
}
