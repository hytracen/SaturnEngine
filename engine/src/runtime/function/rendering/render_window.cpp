...
RenderWindow::RenderWindow(int width, int height, std::string name) : m_width(width), m_height(height), m_name(std::move(name)) {
    glfwInit();
    ...
    glfwSetFramebufferSizeCallback(m_glfw_window, [](GLFWwindow *in_window, int new_width, int new_height) {
        auto *window = reinterpret_cast<RenderWindow *>(glfwGetWindowUserPointer(in_window));
...
// void RenderWindow::CreateSurface(VkInstance instance, VkSurfaceKHR *surface) {
//     if (glfwCreateWindowSurface(instance, m_glfw_window, nullptr, surface) != VK_SUCCESS) {
//         throw std::runtime_error("failed to create window surface!");
//     }
// }

// void RenderWindow::DestroySurface(VkInstance instance, VkSurfaceKHR *surface) {
